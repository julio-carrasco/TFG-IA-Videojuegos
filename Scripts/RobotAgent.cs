...
// Script that controls the robot agent's behavior in the environment.
public class RobotAgent : Agent
{
    ...
    public bool SpawnTargetAtStart = true; //If true, the target will be spawned at the start of each episode
    public bool penalizeTargetReachedGoal = true; //If true, the agent will be penalized if the target reaches its own goal
    ...
    {
        if (SpawnTargetAtStart)
        {
            SpawnTarget(TargetPrefab, transform.position); //spawn target
        }
        else
        {
            m_Target = TargetPrefab; //use the prefab as the target
        }

        if (penalizeTargetReachedGoal)
        {
            ScientistAgent.reachedGoal.AddListener(FailedMission);
        }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        body.rotation = Quaternion.Euler(Random.Range(-30.0f, 30.0f), Random.Range(0.0f, 360.0f), 0);
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        {
            Debug.LogError("m_OrientationCube is null in CollectObservations!");
        }
        if (m_Target == null)
        {
            Debug.LogError("m_Target is null in CollectObservations!");
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        //print(matchSpeedReward * lookAtTargetReward);
